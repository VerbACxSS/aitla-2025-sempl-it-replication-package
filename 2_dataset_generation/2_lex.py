...
PROMPT = load_prompt('2_lex')
FEW_SHOTS = load_few_shot('2_lex')
...
if __name__ == "__main__":
    ...
    print("Train...")
    train_ids = [BatchSimplifier.parse_response(x)[0] for x in read_jsonl('api_files/1_proofreading_train_output.jsonl')]
    train_docs = [BatchSimplifier.parse_response(x)[1] for x in read_jsonl('api_files/1_proofreading_train_output.jsonl')]
    ...
    write_jsonl('api_files/2_lex_train_input.jsonl', train_output)
    print_estimation(PROMPT, FEW_SHOTS, train_docs)

    print("Val...")
    val_ids = [BatchSimplifier.parse_response(x)[0] for x in read_jsonl('api_files/1_proofreading_val_output.jsonl')]
    val_docs = [BatchSimplifier.parse_response(x)[1] for x in read_jsonl('api_files/1_proofreading_val_output.jsonl')]
    ...
    write_jsonl('api_files/2_lex_val_input.jsonl', val_output)
