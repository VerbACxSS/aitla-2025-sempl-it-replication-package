...
if __name__ == "__main__":
    ...
    print("Train...")
    train_ids = [BatchSimplifier.parse_response(x)[0] for x in read_jsonl('api_files/6_nominalizations_train_output.jsonl')]
    train_docs = [BatchSimplifier.parse_response(x)[1] for x in read_jsonl('api_files/6_nominalizations_train_output.jsonl')]
    ...
    write_jsonl('api_files/7_verbs_train_input.jsonl', train_output)
    print_estimation(PROMPT, FEW_SHOTS, train_docs)

    print("Val...")
    val_ids = [BatchSimplifier.parse_response(x)[0] for x in read_jsonl('api_files/6_nominalizations_val_output.jsonl')]
    val_docs = [BatchSimplifier.parse_response(x)[1] for x in read_jsonl('api_files/6_nominalizations_val_output.jsonl')]
    ...
    write_jsonl('api_files/7_verbs_val_input.jsonl', val_output)
